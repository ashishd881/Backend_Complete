...
//this has a problem when we will dave photo then also password will be encrypted so if condition is used

userSchema.method.isPasswordCorrect = async function (password) {     //custom methods banaliya isPasswordCorrect 
   return  await bcrypt.compare(password,this.passsword)                  //true aur false dega user ka diya password password me hai aur this.passsword encrypted wala hai
}

userSchema.methods.generateAccessToken = function(){     //these are jwt tokens
    return jwt.sign({                //sign method token generare kar diya
